# ## BETTER ALGO 2 (dissimilar signatures -> low match accuracy, similar signatures -> high match accuracy, similar but bit different -> medium match accuracy)
...
#     sk = skeletonize((mask > 0).astype(bool)).astype(np.uint8) * 255
#     return sk
...
#     return float(np.exp(-3.0 * m))  # in [0..1], sharper around small distances
...
# def _orb_inlier_ratio(a: np.ndarray, b: np.ndarray) -> float:
#     # use blurred masks (texture-lite but better than pure edges)
#     aa = cv2.GaussianBlur(a, (5, 5), 0)
#     bb = cv2.GaussianBlur(b, (5, 5), 0)
...
#     if len(good) < 6:
#         return 0.0
#     src = np.float32([k1[i].pt for i, _ in good]).reshape(-1, 1, 2)
#     dst = np.float32([k2[j].pt for _, j in good]).reshape(-1, 1, 2)
#     M, inliers = cv2.estimateAffinePartial2D(src, dst, method=cv2.RANSAC,
#                                              ransacReprojThreshold=3.0, maxIters=2000)
#     if inliers is None:
#         return 0.0
#     inlier_count = int(inliers.sum())
#     base = max(10, min(len(k1), len(k2)))
#     return float(min(1.0, inlier_count / base))
...
#     a2 = (m2 > 0).sum()
#     if a1 > 0 and a2 > 0:
#         s = float(np.clip(np.sqrt(a1 / a2), 0.8, 1.25))
#         m2 = _affine_scale(m2, s)

#     # translate by centroids
#     c1x, c1y = _center_of_mass(m1)
#     c2x, c2y = _center_of_mass(m2)
#     m2 = _affine_translate(m2, c1x - c2x, c1y - c2y)

#     # small rotation sweep; choose best Dice on skeletons (tolerant)
#     best = m2
#     best_score = -1.0
#     sk1 = _skeleton(m1)
#     for ang in range(-8, 9, 2):
#         rot = _rotate(m2, float(ang))
#         score = _dice(sk1, _skeleton(rot), r=1)
#         if score > best_score:
#             best_score = score
#             best = rot
#     return best
...
#     # Prevent false boosts when all cues weak
#     if (dice < 0.20) and (chamfer < 0.20) and (iou < 0.20):
#         base *= 0.5

#     accuracy = float(np.clip(base * 100.0, 0.0, 100.0))
#     breakdown = {
#         "dice": round(dice, 3), "iou": round(iou, 3),
#         "chamfer": round(chamfer, 3), "shape": round(shape_sim, 3),
#         "hu": round(hu, 3), "orb": round(orb, 3)
#     }
